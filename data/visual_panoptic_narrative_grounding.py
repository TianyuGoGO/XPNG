...
class PanopticNarrativeGroundingDataset(Dataset):
    ...
    def __init__(self, cfg, split, train=True):
        ...
        self.panoptic_narrative_grounding = np.array([
            ln
    ...
    def __getitem__(self, idx):
        ...
        aattn_mask=torch.zeros(64)
        aattn_mask[:len(noun_vector_padding.nonzero( as_tuple=False))]=1
        # aattn_mask=torch.ones(64)
        # aattn_mask[:len(noun_vector_padding.nonzero( as_tuple=False))]=0
        
        
        
        #_________________________________add 
        attn_mask=torch.zeros((1,64, 64))
        attn_mask1=torch.zeros((1,64,64))
        attn_mask[:,:,:]=-1e9
        
        attn_mask[0,:len(noun_vector_padding.nonzero( as_tuple=False)),:len(noun_vector_padding.nonzero( as_tuple=False))]=0
        attn_mask1[0,:len(noun_vector_padding.nonzero( as_tuple=False)),:len(noun_vector_padding.nonzero( as_tuple=False))]=1
        attn_mask=attn_mask.repeat(8,1,1)
        attn_mask1=attn_mask1.repeat(8,1,1)
        return caption, grounding_instances, \
            ann_categories, ann_types, noun_vector_padding, ret_noun_vector, fpn_data,attn_mask,attn_mask1, aattn_mask
...
class PanopticNarrativeGroundingValDataset(Dataset):
    ...
    #     img=torch.nn.functional.interpolate(img, size=(640,640), scale_factor=None, mode='bilinear', align_corners=None)
        
      
        return caption, grounding_instances, \
             ann_categories, ann_types_valid, noun_vector_padding, ret_noun_vector, fpn_data,attn_mask,attn_mask1,aattn_mask,img